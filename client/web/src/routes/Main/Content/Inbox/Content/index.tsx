...
export const InboxContent: React.FC = React.memo((props) => {
  ...
  if (!inboxItem) {
    return (
      <div className={styles.notFound}>
        <Icon className={styles.notFoundIcon} icon="mdi:email-search-outline" />
        <div className={styles.notFoundText}>{t('没有找到该记录')}</div>
      </div>
    );
  }
  ...
  return (
    <div className={styles.notFound}>
      <Icon className={styles.notFoundIcon} icon="mdi:alert-circle-outline" />
      <div className={styles.notFoundText}>{t('没有找到该类型的渲染方式')}</div>
    </div>
  );
});
