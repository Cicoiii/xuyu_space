...
/**
 * 清空会话消息
 * @param type 'local' - 仅本地清空 | 'global' - 全局清空（需权限）
 */
export async function clearConverseMessages(
  converseId: string,
  type: 'local' | 'global' = 'local'
): Promise<{ success: boolean; type: string; converseId: string }> {
  const { data } = await request.post('/api/chat/message/clearConverseMessages', {
    converseId,
    type,
  });
  return data;
